...
<body>
			...
			<table class="joinForm">
				...
				<tr>
					<td class="sub"><span>비밀번호</span> <img
						src="resources/user/ico_required.png"></td>
					<td class="body">
						<input type="password" id="objPWD" name="pwd" value="" maxlength="16">
						<span>(영문&nbsp;대소문자/숫자/특수문자 중 2가지 이상 조합,&nbsp;10자~16자)</span></br>
				...
				<tr>
					...
					<input type="text" id="objTell_mid" value="" size="4" maxlength="4" name="tel2"> 
					<span>-</span> <input type="text"
						id="objTell_div" value="" size="4" maxlength="4" name="tel3"></td>
				</tr>
				<tr>
					<td class="sub"><span>휴대전화</span> <img
						src="resources/user/ico_required.png"></td>
					<td class="body">
					<select id="objCell_region" style="height: 25px;" name="phone1">
							<option value="010">010&nbsp;&nbsp;</option>
							<option value="011">011</option>
							<option value="016">016</option>
							<option value="017">017</option>
				...
				<tr>
					<td class="sub"><span>이메일</span> <img
						src="resources/user/ico_required.png"></td>
					<td class="body">
					<input type="text" id="objEmail_ID" value=""  maxlength="30" name="email1">
						<span>@</span> 
					<input type="text" id="objEmail_domain" value="" maxlength="15"
						readonly="readonly" name="email2"> 
						
						<select id="objDomain_List"
	...
	이 약관은 hisfit[히즈핏] 회사(전자상거래 사업자)가 운영하는 hisfit[히즈핏] 사이버 몰(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리?의무 및 책임사항을 규정함을 목적으로 합니다.
	
	※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다」
	
	제2조(정의)
	①“몰” 이란 hisfit[히즈핏] 회사가 재화 또는 용역(이하 “재화등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터등 정보통신설비를 이용하여 재화등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.
	...
	히즈핏 쇼핑몰은 회원가입, 주문안내, 배송안내 등 비광고성 정보를 카카오톡 알림톡으로 알려드리며, 만약 알림톡 수신이 불가능하거나 수신 차단하실 경우에는 일반 문자 메세지로 보내드립니다.
	...
	선택 약관에 동의하지 않으셔도 회원가입은 가능하며, 회원가입 후 회원정보수정 페이지에서 언제든지 수신여부를 변경하실 수 있습니다.
				...
				<tr>
					<td colspan="2">
						<div>
							<button class="okBtn" id="btnOK" type="button">회원가입</button>
							<button class="cancelBtn" id="btnCancel" type="button">취소</button>
						</div>
					</td>
				</tr>
			</table>
	...
	check.css("color","red");
	 if(pw.length < 10 || pw.length > 17){
		  ...
		  //alert("10자리 ~ 16자리 이내로 입력해주세요.");
			check.text("10자리 ~ 16자리 이내로 입력해주세요.");
			return false;
	 ...
	 if( (num < 0 && eng < 0) || (eng < 0 && spe < 0) || (spe < 0 && num < 0) ){
			...
			check.text("영문,숫자, 특수문자 중 2가지 이상을 혼합하여 입력해주세요.");
			return false;
	 	}
...
$('#objEmail_ID').keyup(function(event){
	if(!(event.keyCode >=37 && event.keyCode <=40)){
		var inputVal = $(this).val();
		$(this).val(inputVal.replace(/[^a-z0-9+]/gi,''));
	}
...
$('#objEmail_domain').keyup(function(event){
	if(!(event.keyCode >=37 && event.keyCode <=40)){
		var inputVal = $(this).val();
		$(this).val(inputVal.replace(/[^a-z0-9+.]/gi,''));
		$(this).val($(this).val().toLowerCase());
...
function checkInfo(){
	...
	if($("#objAdress_detail").val() == ""){
		alert("상세 주소를 입력해주세요");
		return false;
	}

	if($("#objCell_mid").val() == "" || $("#objCell_div").val() == ""){
		alert("휴대전화번호를 입력해주세요");
		return false;
	}

	if($("#objEmail_ID").val() == "" || $("#objEmail_domain").val() == ""){
		alert("이메일을 입력해주세요");
		return false;
	}
